...
proto.add = function(item, index) {
  ...
  if( title ) title = '<div class="xw-tab-item-title">' + title + '</div>'
...
proto.active = function(id) {
  ...
  if( ~self.indexOf(view) ) {
    if( self._selected !== view ) {
      self._selected && self._selected.fire('deactivate');
      view.fire('activate');
      self.fire('change', {prev: self._selected, current:view});
    }
    self._selected = view || self._selected;
  }
...
proto.validate = function() {
  ...
  if( !~index ) {
    selected = self._selected = self.get(0);
